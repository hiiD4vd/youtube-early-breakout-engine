...
type MarketVideo = { video_id: string; title: string | null; channel_title: string | null; video_url: string; thumbnail_url: string | null; category_id: string | null; view_count: number; source_rank: number | null };
type MarketResponse = { items: MarketVideo[]; methodology: string };
...
export default function TopicTrendsPage() {
  ...
  const { data: market } = useSWR<MarketResponse>("/api/v1/youtube/market/videos", fetcher, { refreshInterval: 60_000 });
  ...
  return <div className="mx-auto max-w-7xl">
    ...
    {market && <section className="mt-8"><div className="flex items-baseline justify-between gap-4"><div><h2 className="text-xl font-semibold">Market coverage · public charts</h2><p className="mt-1 text-sm text-text-secondary">Video market yang baru dikumpulkan. Ini adalah input luas untuk Market Trends, belum diklaim sebagai cluster topik.</p></div><span className="font-mono text-sm text-neon">{market.items.length} shown</span></div><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{market.items.slice(0, 12).map((video) => <a key={video.video_id} href={video.video_url} target="_blank" rel="noreferrer" className="overflow-hidden rounded-xl border border-line bg-surface hover:bg-white/[.03]"><div className="aspect-video bg-bg-secondary">{video.thumbnail_url && <img src={video.thumbnail_url} alt="" className="h-full w-full object-cover" />}</div><div className="p-3"><p className="line-clamp-2 text-sm font-medium">{video.title || video.video_id}</p><p className="mt-1 truncate text-xs text-text-secondary">{video.channel_title || "Unknown channel"}</p><p className="mt-3 font-mono text-xs text-neon">{format.format(video.view_count)} observed views</p></div></a>)}</div><p className="mt-3 text-xs text-text-tertiary">{market.methodology}</p></section>}
