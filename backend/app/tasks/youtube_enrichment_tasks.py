...
def enrich_youtube_breakout(self: Task, video_id: str) -> dict[str, str]:
    pending = SeedStore().get_pending_breakout(video_id)
    if not pending:
    ...
    values = {"channel_id": seed["channel_id"], "channel_title": seed.get("channel_title"), "title": seed.get("title"), "video_url": seed["video_url"], "thumbnail_url": seed.get("thumbnail_url"), "published_at": datetime.fromisoformat(seed["published_at"]), "initial_view_count": seed["seed_view_count"], "current_view_count": pending["current_view_count"], "velocity_per_hour": pending["velocity_per_hour"], "breakout_score": pending["velocity_per_hour"], "peak_timestamp_seconds": pending["peak_timestamp_seconds"], "peak_frame_path": pending["peak_frame_path"], "transcript": transcript, "niche": facts.niche, "visual_facts": {"facts": facts.visual_facts}, "ai_analysis": facts.model_dump(), "raw_metadata": pending}
